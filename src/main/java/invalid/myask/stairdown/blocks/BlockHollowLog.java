...
public class BlockHollowLog extends BlockLog implements IParentBlock {
    ...
    public BlockHollowLog(Block parent, int meta) {
        ...
        if (getUnlocalizedName().startsWith("tile.tile.")) this.setBlockName(getUnlocalizedName().substring(10));
    ...
    public void addCollisionBoxesToList(World worldIn, int x, int y, int z, AxisAlignedBB mask,
        List<AxisAlignedBB> outputList, Entity collider) {
        List<AxisAlignedBB> list = new ArrayList<>();
        int meta = worldIn.getBlockMetadata(x, y, z);
        if ((meta & 12) != 0) {
            list.add(AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + (Config.hollow_thickness / 16D), z + 1));
            list.add(
                AxisAlignedBB.getBoundingBox(x, y + ((16 - Config.hollow_thickness) / 16D), z, x + 1, y + 1, z + 1));
        }
        if ((meta & 12) != 4) {
            list.add(AxisAlignedBB.getBoundingBox(x, y, z, x + (Config.hollow_thickness / 16D), y + 1, z + 1));
            list.add(
                AxisAlignedBB.getBoundingBox(x + ((16 - Config.hollow_thickness) / 16D), y, z, x + 1, y + 1, z + 1));
        }
        if ((meta & 12) != 8) {
            list.add(AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + (Config.hollow_thickness / 16D)));
            list.add(
                AxisAlignedBB.getBoundingBox(x, y, z + ((16 - Config.hollow_thickness) / 16D), x + 1, y + 1, z + 1));
        }
        for (AxisAlignedBB aabb : list) if (aabb.intersectsWith(mask)) outputList.add(aabb);
    }
    ...
    public IIcon getIcon(int side, int meta) {
        int i = inSide;
        if (i == -1 || i != side) return super.getIcon(side, meta);
        else return getInsideIcon(meta);
    }
    ...
    public void getSubBlocks(Item itemIn, CreativeTabs tab, List<ItemStack> list) {
        parentBlock.getSubBlocks(itemIn, tab, list); // ...actually, that's enough, since it doesn't check the itemIn
    }
